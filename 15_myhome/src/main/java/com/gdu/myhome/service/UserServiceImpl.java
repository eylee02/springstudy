...
public class UserServiceImpl implements UserService {
  ...
  private final String client_id = "n23Sahbp0fmkWfStEsou";
  private final String client_secret = "JJjndHwS_u";
  ...
  public void active(HttpSession session, HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      if(insertActiveUserResult == 1 && deleteInactiveUserResult == 1) {
        out.println("alert('휴면계정이 복구되었습니다. 계정활성화를 위해 로그인 해 주세요.')");
        // 로그인 페이지로 보내면 로그인 후 다시 휴면 계정 복구페이지로 돌아오므로 main으로 이동시킨다.
        out.println("location.href='" + request.getContextPath() + "/main.do'");  
      } else {
        out.println("alert('휴면 계정 복구에 실패했습니다. 다시 시도해 주세요.')");
        out.println("history.back()");
      }    
      out.println("</script>");
      out.flush();
