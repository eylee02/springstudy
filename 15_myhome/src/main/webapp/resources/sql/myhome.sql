...
CREATE TABLE FREE_T (
    ...
    STATUS      NUMBER              NOT NULL, -- 1:정상게시글, 0:삭제된게시글 (실제로 삭제되지 않는 게시판)
...
CREATE TABLE BLOG_T (
    ...
    TITLE       VARCHAR2(500 BYTE) NOT NULL,  -- 글제목
    CONTENTS    CLOB               NULL,      -- 글내용
    USER_NO     NUMBER             NOT NULL,  -- 작성자 번호(NULL인 경우 ON DELETE SET NULL 처리가 가능하고, NOT NULL인 경우에 ON DELETE CASCADE 처리가 가능하다)
    HIT         NUMBER             DEFAULT 0, -- 조회수
    IP          VARCHAR2(30 BYTE)  NULL,      -- 작성자 ip주소
...
-- 테스트용 INSERT
INSERT INTO USER_T VALUES(USER_SEQ.NEXTVAL, 'user1@naver.com', STANDARD_HASH('1111', 'SHA256'), '사용자1', 'M', '01011111111', '11111', '디지털로', '가산동', '101동 101호', 0, 0, TO_DATE('20231001', 'YYYYMMDD'), TO_DATE('20221001','YYYYMMDD'));
INSERT INTO USER_T VALUES(USER_SEQ.NEXTVAL, 'user2@naver.com', STANDARD_HASH('2222', 'SHA256'), '사용자2', 'F', '01022222222', '22222', '디지털로', '가산동', '201동 101호', 0, 0, TO_DATE('20230801', 'YYYYMMDD'), TO_DATE('20221001','YYYYMMDD'));
INSERT INTO USER_T VALUES(USER_SEQ.NEXTVAL, 'user3@naver.com', STANDARD_HASH('3333', 'SHA256'), '사용자3', 'F', '01033333333', '33333', '디지털로', '가산동', '101동 101호', 0, 0, TO_DATE('20230601', 'YYYYMMDD'), TO_DATE('20221001','YYYYMMDD'));
...
INSERT INTO FREE_T VALUES(FREE_SEQ.NEXTVAL, 'user2@naver.com', '내용', SYSTIMESTAMP, 1, 0, FREE_SEQ.CURRVAL, 0);
...
INSERT INTO FREE_T VALUES(FREE_SEQ.NEXTVAL, 'user1@naver.com', '내용3', SYSTIMESTAMP, 1, 0, FREE_SEQ.CURRVAL, 0);
...
INSERT INTO FREE_T VALUES(FREE_SEQ.NEXTVAL, 'user2@naver.com', '내용12', SYSTIMESTAMP, 1, 0, FREE_SEQ.CURRVAL, 0);
...
INSERT INTO FREE_T VALUES(FREE_SEQ.NEXTVAL, 'user1@naver.com', '내용15', SYSTIMESTAMP, 1, 0, FREE_SEQ.CURRVAL, 0);
...
SELECT UP.UPLOAD_NO, UP.TITLE, UP.CONTENTS, UP.CREATED_AT, UP.MODIFIED_AT, ATC.PATH, ATC.ORIGINAL_FILENAME, ATC.FILESYSTEM_NAME, ATC.DOWNLOAD_COUNT, ATC.HAS_THUMBNAIL, USR.USER_NO, USR.EMAIL, USR.NAME
...
--        블로그가 없는 사용자도 모두 조인하는 방식(불필요한 조인)

SELECT A.BLOG_NO, A.TITLE, A.CONTENTS, A.USER_NO, A.EMAIL, A.HIT, A.IP, A.CREATED_AT, A.MODIFIED_AT
  FROM (SELECT ROW_NUMBER() OVER(ORDER BY B.BLOG_NO DESC) AS RN, B.BLOG_NO, B.TITLE, B.CONTENTS, B.USER_NO, U.EMAIL, B.HIT, B.IP, B.CREATED_AT, B.MODIFIED_AT
...
-- 2) 블로그 상세 정보 조회(inner join 오라클 문법으로 작성)
...
-- 3) 댓글 목록
-- 블로그 - 댓글 : 1:N (댓글이 달린 블로그 정보는 이미 상세보기에 모두 표시되어 있으므로 조인할 필요가 없다.
-- 사용자 - 댓글 : 1:N (댓글에 사용자 이름을 표시)
