...
<jsp:include page="../layout/header.jsp">
  <jsp:param value="${blog.blogNo}번블로그" name="title"/>
</jsp:include>
      ...
      $.ajax({
        ...
        success: (resData) => {  // resData = {"commentList": [], "paging": "<div>...</div>"}
          ...
          $.each(resData.commentList, (i, c) => {
            ...
            str += '<i class="fa-regular fa-circle-xmark btn_remove_comment">삭제</i>';
            str += '</div>';
              }     
            }
            str += '</div>';           	
        ...
        $.ajax({
          ...
          success: (resData) => {
            if(resData.addCommentReplyResult === 1) {
              alert('답글이 등록되었습니다.');
              fnCommentList();
              frmAddReply.find('textarea').val('');   // 댓글입력창 초기화
            } else {
