...
<script>
  ...
  const fnScroll = () => {
    ...
    if(timerId){  // timerId가 undifined이면 false로 인식, timerId가 값을 가지면 true로 인식
      clearTimeout(timerId);  
    }
     
    timerId = setTimeout(() => {  // setTimeout 실행 전에는 timerId가 undifined 상태, setTimeout이 한 번이라도 동작하면 timerId가 값을 가짐
     ...
     if((scrollTop + windowHeight + 100) >= documentHeight) {  // 스크롤이 바닥에 닿기 50px 전에 true가 됨
      ...
      }, 200);   // 200밀리초(0.2초) 후 동작(시간은 임의로 조정 가능함 1초 이내가 적당함)
