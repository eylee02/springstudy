...
<script>
  ...
  // 페이지를 바꿀 때 마다 호출되는 fnAjaxPaging 함수
  function fnAjaxPaging(p){
    page = p;        // 페이지 번호를 바꾼다.
    fnMemberList();  // 새로운 목록을 가져온다.
  }

  // 회원 정보 상세 조회하기 (조회버튼은 이벤트로 생성되어서 on으로 해야함 click으로 인식안됌)
      ...
      $.ajax({
    	...
    	success: function(resData){
    	  ...
    	  if(!member){
    	    ...
    	    $('#id').val(member.id).prop('disabled', true);   // 조회시 아이디 수정불가
